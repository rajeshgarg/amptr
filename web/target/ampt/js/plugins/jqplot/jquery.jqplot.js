...
(function($) {
    ...
    Series.prototype.toggleDisplay = function(ev) {
		...
		if(ev.data.dataElement){
			s = ev.data.dataElement;
		}
        ...
        if (speed) {
				...
				for(var i=0;i<series.length;i++){
					series[i].shadowCanvas._elem.fadeOut(speed);
					series[i].canvas._elem.fadeOut(speed);
					series[i].canvas._elem.nextAll('.jqplot-point-label.jqplot-series-'+s.index).fadeOut(speed);
				}

...
function renewExpirationDate(reservationExDate, lineItemId){
	...
	if(productClassName && (productClassName == 'HOME PAGE' || productClassName == 'Display Cross Platform') && startDate > modifiedStartDate){
	...
	else if(productClassName && (productClassName != 'HOME PAGE' || productClassName == 'Display Cross Platform') && startDate > now){
...
function showHideRenewBtn(gsr){
	...
	if(reservationExpiryDate != ""){
		...
		if (days <= 3) {
			...
			if(($("#proposalOwnerRole", "#reservationParentDiv").val() == 'PLR') && (productClassName == 'HOME PAGE' || productClassName == 'Display Cross Platform')){
				return false;
			}else{
				return  true;
			}

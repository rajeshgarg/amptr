...
$.fn.jqGrid = function( pin ) {
	...
	return this.each( function() {
        ...
        setPager = function (pgid, tp){
			...
			if(dir=="rtl") { pgl += str; }
			if(ts.p.pginput===true) { pginp= "<td dir='"+dir+"'>"+$.jgrid.format(ts.p.pgtext || "","<input class='ui-pg-input' disabled='true' type='text' size='2' maxlength='7' value='0' role='textbox'/>","<span id='sp_1_"+$.jgrid.jqID(pgid)+"'></span>")+"</td>";}
			if(ts.p.pgbuttons===true) {
		...
		$("th",thr).each(function ( j ) {
			...
			if( !(nm == 'cb' || nm=='subgrid' || nm=='rn') ) {
				if(ts.p.viewsortcols[2] && ts.p.colModel[j].sortable == true){
				...
				$("#dData","#"+dtbl+"_2").click(function(e){
					...
					if(ret[0] && !rp_ge[$t.p.id].processing) {
						...
						var ajaxOptions = $.extend({
							...
							complete:function(data,Status){
								if(Status != "success") {
									if ($.isFunction($t.p.handleDeleteError)) {
										$t.p.handleDeleteError(data, Status);
									}
									else {
										ret[0] = false;
										if ($.isFunction(rp_ge[$t.p.id].errorTextFormat)) {

...
public class VulnerableHomePageReservationJob extends AbstractCronJob {
	...
	protected void executeInternal(JobExecutionContext context)
		...
		for(Long productClassId : productClassIdLst){
			StringBuffer textMsg=new StringBuffer();
		 	List<LineItem> lineItemLst = proposalService.getVulnerableHomepageReservations(productClassId);
			...
			productOwnerroles.add(SecurityUtil.PRODUCT_OWNER);
		   	...
		   	String productClass = (productClassMap.get(productClassId).equals("HOME PAGE") ? ConstantStrings.PRODUCT_CLASS_HOME_PAGE :  ConstantStrings.PRODUCT_CLASS_DISPLAY_CROSS_PLATFORM);
		   	sendMail(proposalUtilService.emailIdOfUserBasedOnRole(adminRoles).toString(), productClass +" Reservations in Vulnerable State",mailCC == null? "": mailCC.toString(),textMsg);
		}
	}
